      ...
      <v-form @submit.prevent="handleSubmit">
        <v-row>
          <v-col cols="12">
            ...
            <DropzoneComponent
              :model-value="creditAssessmentStore.carnetIdentidad"
        ...
        <v-row>
          <v-col cols="12" class="d-flex gap-4">
            <v-btn
              ...
              :disabled="!creditAssessmentStore.carnetIdentidad"
              class="mr-2"
            >
              Verificar
            </v-btn>
            <DownloadComponent
              :file="creditAssessmentStore.carnetIdentidad"
              label="Descargar PDF"
              color="secondary"
              class="ml-2"
            />
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.info('Documento enviado:', creditAssessmentStore.carnetIdentidad)
  } catch (error) {
...
const handleFileUpdate = (file: File | undefined) => {
  creditAssessmentStore.setDocument(DocumentType.CARNET_IDENTIDAD, file)
}
</script>
