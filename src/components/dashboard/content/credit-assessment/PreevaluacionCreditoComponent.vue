<template>
  <DashboardContentBase
    title="Pre-evaluación de crédito"
    description="Evaluación de crédito por Pre-evaluación"
  >
    <v-card-text>
      <p class="text-body-1 mb-6">
        Esta sección permite realizar la pre-evaluación de crédito del cliente.
      </p>

      <v-form @submit.prevent="handleSubmit">
        <v-row>
          <v-col cols="12">
            <div class="text-subtitle-1 mb-2">Documento de Pre-evaluación</div>
            <DropzoneComponent
              :model-value="creditAssessmentStore.preevaluacionCredito"
        ...
        <v-row>
          <v-col cols="12" class="d-flex gap-4">
            <v-btn
              ...
              :disabled="!creditAssessmentStore.preevaluacionCredito"
              class="mr-2"
            >
              Verificar
            </v-btn>
            <DownloadComponent
              :file="creditAssessmentStore.preevaluacionCredito"
              label="Descargar PDF"
              color="secondary"
              class="ml-2"
            />
...
const handleFileUpdate = (file: File | undefined) => {
  creditAssessmentStore.setDocument(DocumentType.PREEVALUACION_CREDITO, file)
}
</script>
